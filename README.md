...
Оркестратор принимает решение о судьбе тендера/проекта в три этапа:
...
## Lessons Learned — институциональная память (3 уровня)

Система учится на каждом обработанном объекте:

1. **База уроков** — PostgreSQL + pgvector. Каждый урок: лот, вид работ, агент, категория, нормативка, эмбеддинг.
2. **RAG-инъекция** — при анализе нового объекта релевантные уроки подставляются в промпт агента.
3. **Skill Mutation** — после N подтверждений (порог=3) урок превращается в автоматическое правило без RAG-поиска.
...
| **ФЗ-223** | Закупки отдельными видами юрлиц |
