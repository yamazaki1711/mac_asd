...
async def asd_generate_protocol(
    ...
    v11.2.2: Генерирует полноценный DOCX с 3-колоночной таблицей:
...
async def asd_add_trap(
    ...
    v11.2.2: Добавлены поля channel, category, court_cases для интеграции
...
async def asd_list_telegram_channels(
    ...
    v11.2.2: Показывает все каналы из config/telegram_channels.yaml
...
async def asd_ingest_telegram(
    ...
    v11.2.2: Поддерживает каталог каналов для автокатегоризации
