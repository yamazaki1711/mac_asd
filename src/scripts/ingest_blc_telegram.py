...
# Basic system prompt for extracting legal traps from messy Telegram text.
EXTRACTION_PROMPT = """
...
async def process_telegram_post(db: Session, text: str, source_name: str) -> bool:
    """Sends the text to Ollama to extract BLC trap, inserts if valid."""
    if len(text.strip()) < 50:
        return False # Too short to be a valid case study
    
    # 1. Ask Ollama to extract
    ...
    embedding = await ollama_client.embeddings(data.get("description", ""))
    ...
    try:
        for msg in messages:
            ...
            if isinstance(text_entities, list):
                # Combine elements, sometimes it's text, sometimes dict with type
                extracted_text = ""
            ...
            if extracted_text:
                processed_count += 1
                is_saved = await process_telegram_post(db, extracted_text, f"Telegram: {chat_name}")
                if is_saved:
                    traped_count += 1
                    
        logger.info(f"Finished processing. Evaluated {processed_count} texts, found {traped_count} Traps.")
    finally:
        db.close()
