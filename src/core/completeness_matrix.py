...
Usage:
    from src.core.completeness_matrix import verify_completeness, LinkStatus
...
class ChainLink:
    ...
    drawing_status: LinkStatus = LinkStatus.UNVERIFIED
    drawing_ref: Optional[str] = None           # Ссылка на чертеж

    spec_status: LinkStatus = LinkStatus.UNVERIFIED
    spec_ref: Optional[str] = None              # Ссылка на спецификацию

    cert_status: LinkStatus = LinkStatus.UNVERIFIED
    cert_ref: Optional[str] = None              # Ссылка на сертификат/паспорт

    aosr_status: LinkStatus = LinkStatus.UNVERIFIED
    aosr_ref: Optional[str] = None              # Ссылка на АОСР

    ks2_status: LinkStatus = LinkStatus.UNVERIFIED
    ...
    def build(self, vor_positions: List[Dict]) -> CompletenessResult:
        ...
        for pos in vor_positions:
            ...
            if drawing_ref and drawing_ref in self._drawings:
                chain.drawing_status = LinkStatus.VERIFIED
                chain.drawing_ref = drawing_ref
            elif drawing_ref:
                chain.drawing_status = LinkStatus.MISSING
            ...
            if spec_linked:
                chain.spec_status = LinkStatus.VERIFIED
                chain.spec_ref = spec_ids[0]
            else:
                chain.spec_status = LinkStatus.MISSING
        ...
        verified = sum(
            1 for c in chains
            if all(s == LinkStatus.VERIFIED for s in [
