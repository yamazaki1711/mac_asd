...
ASD_IRRELEVANT = [
    ...
    # Рекламные паттерны (пропущены в NOISE_CATEGORIES)
    "скидка", "акция", "спецпредложение",
    "оставьте заявку", "подключить подписку", "годовую подписку",
    "только сегодня", "доступ на 3 дня", "бесплатный доступ",
    "менеджер предложит", "максимальной скидкой",
    # Агрегаторы / папки / подборки (не контент)
    "собрала папку", "добавляйте папку", "папку с экспертами",
    "сохранить капитал", "инвестиций",
]
