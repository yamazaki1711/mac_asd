...
## 10. Что меняется в кодовой базе
...
1. **Evidence Graph v2** — ядро, без которого всё остальное не имеет смысла (~600 LOC)
2. **Inference Engine** — правила 1-6 (~400 LOC) + typical_rates.yaml
...
**Итого: ~2250 строк нового кода.** Вся бизнес-логика инференса — symbolic (правила, не LLM). LLM используется только агентами для генерации текстов документов и анализа содержания.
