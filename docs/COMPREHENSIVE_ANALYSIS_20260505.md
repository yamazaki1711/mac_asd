...
| Alembic-миграций | **2** |
| MCP-инструментов (реализовано) | **8** |
...
| 20 | `src/core/telegram_bot.py` | — | Telegram Bot: приём WorkEntry |
...
**Pass rate: 97.5%** (590/605, dev_linux)
...
|-----------|-----------|-----------|-----|
| Тестов (passed) | 590 (dev_linux) | 752 (mac_studio) | +162 (MLX-зависимые) |
...
- **+162 теста** (752 passed на mac_studio, 98.0% pass rate)
- **+5,873 строк кода**
...
**Ключевой вывод:** Система имеет мощный backend (8 MCP-инструментов, 590 тестов (dev_linux), 8 агентов) и теперь — работающий фронтенд (Flask web UI, 6 страниц). Основные пробелы: Alembic-миграции, Google OAuth, Knowledge Invalidation index, и документация (которая актуализируется этим документом).
