...
Полная схема реляционной базы данных АСД: таблицы, столбцы, типы,
связи, индексы, триггеры. Включая векторные embeddings для pgvector.
...
```sql
CREATE EXTENSION IF NOT EXISTS vector;    -- pgvector: векторный поиск
CREATE EXTENSION IF NOT EXISTS pg_trgm;   -- trigram: нечёткий текст
...
Детали контрактов (извлечённые из документов).
...
CREATE TABLE contracts (
    ...
    date            DATE,
    party_1         VARCHAR(200),                         -- Заказчик
    party_1_inn     VARCHAR(12),
    party_2         VARCHAR(200),                         -- Подрядчик
    party_2_inn     VARCHAR(12),
...
CREATE TABLE traps (
    ...
    category        VARCHAR(30)  NOT NULL,                -- "payment" | "penalty" | "acceptance" | "scope" | "warranty"
...
Найденные ловушки в конкретных документах.
...
Позиции ВОР (ведомости объёмов работ).
...
Акты (АОСР, входной контроль, скрытые работы).
...
-- ... аналогично для contracts, claims, lawsuits, estimates, acts, letters, etc.
```
...
RETURNS TRIGGER AS $$
BEGIN
    -- Вызывается при каждом записи в audit_log
...
|---------|-------|--------|
| documents | ~200 | 50 MB |
| chunks | ~8000 | 500 MB (включая embeddings) |
| contracts | ~10 | 1 MB |
| traps | ~27 | 1 MB |
| trap_matches | ~200 | 5 MB |
| claims | ~5 | 1 MB |
| lawsuits | ~2 | 1 MB |
| vor | ~30 | 5 MB |
| vor_items | ~15000 | 10 MB |
| estimates | ~20 | 10 MB |
| estimate_items | ~3000 | 5 MB |
| acts | ~100 | 5 MB |
| letters | ~50 | 5 MB |
...
Документ актуализирован. SQLAlchemy модели реализованы в src/db/models.py. Схема будет расширяться по мере реализации Packages 2-10.
