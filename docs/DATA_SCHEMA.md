# АСД v11.3 — СХЕМА ДАННЫХ

**Дата:** 20 апреля 2026
**Статус:** Активная разработка (Package 1 завершён, Package 4 частично реализован)
**СУБД:** PostgreSQL 16 + pgvector + pg_trgm
**Модели:** Gemma 4 31B 4-bit (ПТО/Юрист/Сметчик/Закупщик/Логист), Gemma 4 E4B 4-bit (Делопроизводитель), Llama 3.3 70B 4-bit (Hermes/PM), bge-m3-mlx-4bit (embeddings)

---

## 1. ОБЗОР

Полная схема реляционной базы данных АСД v11.3: таблицы, столбцы, типы, связи, индексы, триггеры. Включая векторные embeddings для pgvector (bge-m3, 1024 dim). Схема поддерживает пять рабочих агентов (Юрист, ПТО, Сметчик, Закупщик, Логист) на Gemma 4 31B, Делопроизводителя на Gemma 4 E4B и координирующего агента Hermes (PM) на Llama 3.3 70B. Данные между агентами передаются через общий shared memory, основанный на таблицах projects, contracts и ProtocolPartyInfo.

Ключевое расширение v11.3 — полноценная поддержка ProtocolPartyInfo в таблице contracts. Поля реквизитов (ОГРН, адрес, расчётный счёт, банк, БИК, подписант, должность) для обеих сторон договора теперь являются неотъемлемой частью схемы и используются при генерации протоколов разногласий, претензий, исковых заявлений и деловых писем. Это устраняет необходимость повторного извлечения реквизитов и обеспечивает единый источник правды (single source of truth) для всех агентов.
...
- Llama 3.3 70B 4-bit: ~40 GB (Hermes/PM, отдельный профиль)
...
| `docs/ARCHITECTURE.md` | Архитектура АСД v11.3 | LLMEngine → профили mac_studio (Gemma 4 31B), dev_linux (Gemma 4 31B), hermes_pm (Llama 3.3 70B); shared memory → таблицы projects, contracts |
...
Документ актуализирован (v11.3, 20 апреля 2026). SQLAlchemy модели реализованы в `src/db/models.py`. Схема будет расширяться по мере реализации Packages 2-10. ProtocolPartyInfo интегрирован в таблицу contracts и является единым источником реквизитов для всех агентов.
