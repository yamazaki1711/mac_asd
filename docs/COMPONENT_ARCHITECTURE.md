# АСД v11.3 — ДЕТАЛЬНАЯ АРХИТЕКТУРА КОМПОНЕНТОВ
...
Этот документ описывает внутреннюю архитектуру каждого компонента АСД v11.3:
...
|--------|-----------|-----|--------|
| Llama 3.3 70B 4-bit (Hermes) | Оркестратор (pm) | ~40 GB | MLX |
...
| minicpm-v | Vision/OCR (on-demand) | ~5 GB | Ollama |

**Ключевое архитектурное решение v11.3:** Все 5 рабочих агентов (pto, smeta, legal,
...
через EventManager и LangGraph. Сервер построен на фреймворке FastMCP и
...
│   ├── EventManager() → конечный автомат проекта
...
Запуск:
  ...
  6. Инициализация LegalService, EventManager
...
обработки ошибок на стороне Hermes. Формат включает флаг успеха, машинно-читаемый
...
│   │
│   ├── config = settings.get_model_config("vision") → {"engine": "ollama", "model": "minicpm-v"}
│   ├── image_base64: base64-кодированное изображение
│   ├── prompt: текстовая инструкция для модели
│   └── Использует minicpm-v (Ollama) — подгружается по требованию
│
...
> **Примечание:** Начиная с v11.3, модельный роутинг полностью интегрирован
...
Агент (имя роли)
  ...
  │
  ├── vision (Vision/OCR)
  │     → minicpm-v (Ollama, подгружается по требованию)
  │
  └── embed (Embeddings)
...
minicpm-v недоступен (vision/OCR)
...
В v11.3 базовое потребление памяти составляет ~66 GB для моделей, что оставляет
...
└── Модельные бюджеты (фактическое потребление на Mac Studio):
    ...
    └── minicpm-v:              5.0 GB  (vision/OCR, подгружается по требованию)
```

### 5.3. Политики

Базовая загрузка моделей v11.3: ~66.3 GB (52% от 128 GB).

| Давление памяти | % RAM | GB | Действие |
|----------------|-------|-----|----------|
| **low** | < 70% | < 90 GB | Llama 70B + Gemma 4 31B + Gemma 4 E4B + bge-m3 загружены, minicpm-v по требованию |
| **medium** | 70-80% | 90-102 GB | Нормально, пик контекста укладывается, лог-предупреждение |
| **high** | 80-90% | 102-115 GB | minicpm-v выгружается, задачи завершаются последовательно |
...
OCR (minicpm-v):           +5.0 GB  (подгружается при необходимости)
...
через PyMuPDF, затем fallback на OCR через minicpm-v для сканированных страниц.
...
PyMuPDF: extract text per page
  ...
  └── Страница не содержит текста (скан / изображение)
        ...
        │
        ▼
        Подгрузка minicpm-v через RamManager (5 GB)
          │
          ▼
        Ollama Vision API (minicpm-v, Q4_K_M)
          ├── Рендеринг страницы: 300 DPI → PNG
          ├── Base64 кодирование изображения
          ├── Отправка в minicpm-v с OCR-промптом
          └── Возврат распознанного текста
          │
          ▼
        Выгрузка minicpm-v из памяти (keep_alive=0)
```
...
Начиная с v11.3 используется двухуровневый подход: YAML-файлы для базовых
...
### 8.4. Гибридный подход: YAML + RAG/pgvector (с v11.3)

До v11.3 БЛС работала исключительно на YAML-файлах — фиксированный набор
ловушек, обновляемый вручную. Начиная с v11.3 реализован гибридный подход:
...
(например, «Тендер выигран»), EventManager генерирует события для следующих
этапов («Начать подготовку ИД»). Реализован как конечный автомат (State Machine)
на основе NetworkX-графа.

EventManager является связующим звеном между MCP Server (инструменты) и
LangGraph Workflow (оркестрация агентов). Каждый инструмент MCP может
зарегистрировать событие через EventManager, а LangGraph использует
...
```
   ...
   (7 модулей)      (9 модулей)     (3 модуля)
        ...
        │         ┌──────┴──────┬────────────┐
        │         │             │            │
        │    LLMEngine     LightRAG    EventManager
...
│   │   ├── event_manager.py     # EventManager — конечный автомат проекта
│   │   ├── services/
...
│   ├── CONCEPT_v11.md             # Концепция АСД v11
...
*Этот документ описывает внутреннюю архитектуру всех компонентов АСД v11.3.
