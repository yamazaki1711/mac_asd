...
```
_chunk_text(text, chunk_size=12000, chunk_overlap=2400)
  │
  ├── Размер чанка: 12000 символов (20% overlap — индустриальный стандарт)
  ├── Overlap: 2400 символов (контекст между чанками)
  ├── Структурные границы: разделы (нумерованные заголовки) > таблицы (markdown) > абзацы
...
Документы до 280K символов (128K токенов) анализируются целиком за один вызов LLM
...
│   │
│   ├── MAP Stage:
│   │   ├── _chunk_text(text, chunk_size=12000, overlap=2400)
