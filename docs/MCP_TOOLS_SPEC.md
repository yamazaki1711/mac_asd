...
**Дата:** 2 мая 2026
**Статус:** Package 5 ✅ (Evidence Graph, Inference Engine, ProjectLoader), Package 11 ✅ (Chain Builder, HITL, Journal Reconstructor)
...
*   Общее количество инструментов: **74+** (66+ в mcp_servers/asd_core/, включая Evidence Graph, Chain Builder, HITL, Journal Reconstructor).
*   Агенты: Руководитель проекта (PM, Llama 3.3 70B), ПТО, Сметчик, Юрист, Закупщик, Логист (все — Gemma 4 31B), Делопроизводитель (Gemma 4 E4B).
...
| `check_bls` | bool | ❌ | Проверка по БЛС 58 ловушек (по умолч. true) |
...
**Описание:** Проверка статуса всех компонентов системы. Возвращает информацию о загруженных моделях, состоянии БД, потреблении памяти и готовности модулей. Отражает текущую архитектуру с Gemma 4 31B как основной рабочей моделью и Llama 3.3 70B как моделью Руководитель проекта (PM).
...
## 9. EVIDENCE GRAPH v2 (Package 5) — 6 инструментов
...
## 10. CHAIN BUILDER (Package 11) — 3 инструмента
...
## 11. HITL (Package 11) — 3 инструмента
...
## 12. JOURNAL RECONSTRUCTOR (Package 11) — 3 инструмента
...
### 13.1. Маршрутизация LLM-вызовов
...
### 13.5. БЛС (Библиотека Ловушек Сторон)

АСД v12.0 содержит **58 ловушек** в 10 категориях:
...
Документ актуализирован для АСД v12.0 (2 мая 2026). Спецификации инструментов используются при реализации MCP-тулов в Packages 2-12. Добавлены Package 5 (Evidence Graph, Inference Engine, ProjectLoader) и Package 11 (Chain Builder, HITL, Journal Reconstructor). Все агенты используют Gemma 4 31B через разделяемую память LLMEngine; Руководитель проекта использует Llama 3.3 70B. Визионный анализ выполняется Gemma 4 31B VLM по требованию.
