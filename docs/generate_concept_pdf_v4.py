#!/usr/bin/env python3
"""
MAC ASD v12.0.0 — Навыки агентов и рабочие процессы (v4).
...
class CoverPage(Flowable):
    ...
    def draw(self):
        ...
        # Hero: "v12.0.0" in accent color
        c.setFillColor(ACCENT)
        c.drawString(72 + c.stringWidth('MAC ASD ', 'Calibri-Bold', 40), h - 150, 'v12.0.0')
        ...
        meta_values = ['v12.0.0', 'Апрель 2026', 'Mac Studio M4 Max 128GB / MLX', 'LangGraph + LLMEngine + pgvector']
...
def build_content():
    ...
    story.append(p(
        'MAC ASD v12.0.0 (Multi-Agent Construction AI System for Automated Subcontracting Decisions) '
    ...
    story.append(p(
        'Ключевым нововведением версии v12.0.0 является VerdictEngine - взвешенная модель принятия '
        ...
        '61 ловушка, распределённых по 10 категориям, включая новую категорию "Процедурные ловушки". '
    ...
    story.append(Paragraph('Таблица 1. Обзор агентов MAC ASD v12.0.0', CAPTION_STYLE))
    ...
    story.append(p(
        'В версии v12.0.0 Hermes дополнен VerdictEngine - взвешенной моделью принятия решений, '
    ...
    story.append(p(
        'В версии v12.0.0 ПТО получил confidence_score для каждой позиции ВОР, что позволяет Hermes '
    ...
    story.append(p(
        'В версии v12.0.0 Сметчик получил поддержку версионирования ФЕР: система способна '
    ...
    story.append(p(
        'Агент Юрист является одним из наиболее развитых агентов в текущей реализации (v12.0.0). '
        ...
        'содержит 61 ловушка, распределённых по 10 категориям: платежи (8), штрафы (7), приёмка (6), '
    ...
    story.append(p(
        'В версии v12.0.0 структура БЛС расширена новыми полями: precedent_id (ссылка на дело в '
    ...
    story.append(p(
        ...
        'и выбор поставщиков на основе данных от Логиста. В версии v12.0.0 Закупщик получил '
    ...
    story.append(p(
        ...
        'В версии v12.0.0 Логист получил навык автоматического обновления рейтингов поставщиков '
    ...
    story.append(p(
        'Архитектура v12.0.0 вводит параллельные ветки выполнения для оптимизации времени обработки '
    ...
    story.append(p(
        ...
        'В версии v12.0.0 добавлена эскалационная лестница для неоплаченных счетов, автоматизация '
    ...
    story.append(p(
        ...
        'могут отсутствовать при определённых условиях (например, при пропуске ПТО). Версия v12.0.0 '
    ...
    story.append(p(
        'БЛС (База Ловушек Подрядчика) в версии v12.0.0 содержит 61 ловушка, распределённых '
        'по 10 категориям. Это значительное расширение по сравнению с версией v12.0.0, которая '
    ...
    story.append(Paragraph('Таблица 13. Категории БЛС v12.0.0 (61 ловушка, 10 категорий)', CAPTION_STYLE))
    story.append(Spacer(1, 12))

    story.extend(add_h2('12.2. Новые поля'))
    story.append(p(
        'В версии v12.0.0 структура записей БЛС расширена тремя новыми полями, значительно '
    ...
    story.append(p(
        'Категория "Процедурные ловушки" является новой в v12.0.0 и охватывает риски, связанные '
    ...
    story.append(p(
        'В версии v12.0.0 добавлена стратегия MLX offloading: при нехватке памяти для '
    ...
    story.append(Paragraph('Таблица 14. Стек технологий MAC ASD v12.0.0', CAPTION_STYLE))
    story.append(Spacer(1, 12))
    ...
    story.append(p(
        ...
        'которые не могут быть обработаны через PyMuPDF на Stage 1. В версии v12.0.0 Stage 2 '
    ...
    story.append(p(
        ...
        'и приоритеты бизнес-логики. Версия v12.0.0 существенно пересматривает приоритеты: '
        ...
        'Юрист уже реализован (v12.0.0), но его расширение запланировано на P5.'
    ))
    ...
    story.append(Paragraph('Таблица 15. Дорожная карта разработки v12.0.0', CAPTION_STYLE))
...
doc = TocDocTemplate(
    ...
    title='MAC ASD v12.0.0 - Навыки агентов и рабочие процессы',
    author='Z.ai',
    creator='Z.ai',
    subject='Концептуальный документ MAC ASD v12.0.0',
)
