# АСД v12.0 — Жизненный цикл строительства: ASD Workflow
...
Архитектурная система документооборота (АСД) v12.0 спроектирована как **мобильный автономный офис антикризисного управления** для строительного подрядчика. В условиях строительного кризиса — когда Заказчик задерживает оплату, Генподрядчик перекладывает риски, а документооборот ведётся хаотично — АСД обеспечивает непрерывный документальный след, своевременное выявление ловушек и автоматическую генерацию процессуальных документов. Система работает полностью автономно на Mac Studio M4 Max 128GB без зависимости от облачных сервисов, что критично в условиях нестабильной связи на строительных площадках.
...
Данный документ описывает строгий конвейер документооборота и распределение обязанностей между 8 агентами (Руководитель проекта, Юрист, ПТО, Сметчик, Закупщик, Логист, Делопроизводитель, Аудитор) на протяжении 6 реальных этапов строительства объекта. WorkEntry (цифровой ОЖР) обеспечивает приём данных о выполненных работах через Telegram → триггер генерации АОСР.
...
sequenceDiagram
    ...
    Jurist->>Jurist: Проверка по БЛС 61 ловушка (asd_analyze_contract)
...
- ЮРИСТ: `asd_analyze_contract` (жёсткая проверка тендерного договора на предмет 61 ловушка БЛС в 10 категориях; Gemma 4 31B с 128K контекстом — большинство договоров помещаются целиком без Map-Reduce).
- ЗАКУПЩИК: `asd_analyze_lot_profitability` (расчёт рентабельности с учётом неучтённых объёмов из отчёта ПТО).
- PM (Руководитель проекта): Координация агентов, агрегация оценок рисков, принятие стратегического решения.

> **Интеграция Package 5:** Evidence Graph v2 используется для построения графа связей документов и отслеживания целостности комплекта ИД на этапе тендерного анализа.
...
- PM (Руководитель проекта): Валидация полноты покрытия — все ли ловушки с severity ≥ high отражены в протоколе.

> **Интеграция Package 11:** Journal Reconstructor v2 используется для восстановления утраченных журналов работ по косвенным данным из договорной документации.
...
> **Интеграция Package 11:** HITL System обеспечивает сбор и валидацию данных оператора для заполнения лакун в ВОР и РД.
...
- PM (Руководитель проекта): Отслеживание delta комплектности ИД через `asd_id_completeness`.

> **Интеграция Package 11:** Chain Builder генерирует цепочки АОСР по ТТК для автоматического формирования актов освидетельствования скрытых работ на этапе активного строительства.
...
> **Интеграция Package 11:** HITL System обеспечивает сбор подтверждений от оператора для оформления дополнительных объёмов работ и согласования с заказчиком.
...
- PM (Руководитель проекта): Контроль delta=0 перед передачей ИД; инициирование претензионного конвейера при просрочке.

> **Интеграция Package 11:** Chain Builder финализирует цепочки АОСР для формирования итогового пакета документов при сдаче объекта (КС-2, КС-3, комплект ИД).
...
Документ актуализирован для АСД v12.0 (3 мая 2026). Workflow реализован через LangGraph StateGraph с оркестрацией Руководитель проекта (Llama 3.3 70B). 8 агентов (включая Аудитора). Пять рабочих агентов используют Gemma 4 31B с разделяемой памятью (128K контекст). WorkEntry (Telegram → ОЖР) — цифровой ввод данных о выполненных работах.
