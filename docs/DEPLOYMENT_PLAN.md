# АСД v11.3 — ПЛАН РАЗВЁРТЫВАНИЯ

**Дата:** 20 апреля 2026
**Статус:** Активная разработка (Mac Studio M4 Max 128GB)
...
Этот документ — чек-лист развёртывания АСД v11.3 на Mac Studio M4 Max 128GB. От распаковки до рабочего MCP сервера с 23 инструментами и 7 агентами. Ключевое отличие v11.3: пять рабочих агентов (Юрист, ПТО, Сметчик, Закупщик, Логист) работают через модель **Gemma 4 31B 4-bit** (MLX-VLM, 128K контекст) с разделяемой памятью, Делопроизводитель использует **Gemma 4 E4B 4-bit** (~3GB, 8K контекст), а **Hermes (PM)** использует **Llama 3.3 70B 4-bit**. Gemma 4 31B имеет встроенную vision-поддержку (MLX-VLM), что устраняет необходимость в отдельной OCR-модели.
...
| Llama 3.3 70B 4-bit | 40.0 | Hermes (PM), загружается при запуске, 128K контекст |
...
Ollama больше не требуется для основной работы АСД v11.3 — все модели работают через MLX. Устанавливайте только если нужна совместимость с другими инструментами.
...
- [ ] Llama 3.3 70B 4-bit (~40 GB download, Hermes PM)
...
- [ ] Загрузить 58 ловушек из YAML (10 категорий)
  ```bash
  python scripts/load_traps.py traps/default_traps.yaml
  # Ожидаемый вывод: "Загружено 58 ловушек БЛС (10 категорий)"
  ```
...
- [ ] Создать config/settings.py
  ...
  PM_MODEL = "llama-3.3-70b"          # Hermes (PM, 128K)
...
- [ ] Проверить все 58 ловушек БЛС (10 категорий)
...
- [ ] **Llama 3.3 70B 4-bit** загружена (Hermes PM)
...
- [ ] БЛС загружена (58 ловушек в 10 категориях)
...
Документ актуализирован для АСД v11.3 (20 апреля 2026). При развертывании использовать переменную ASD_PROFILE=mac_studio и конфигурацию из .env. Основная модель — Gemma 4 31B 4-bit (все агенты), PM — Llama 3.3 70B 4-bit (Hermes).
