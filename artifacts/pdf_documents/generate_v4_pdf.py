...
def build_content():
    ...
    story.append(p(
        'В версии v11.3.0 Руководитель проекта дополнен VerdictEngine - взвешенной моделью принятия решений, '
        ...
        'Руководитель проекта получил навыки отката к предыдущим этапам и работы с неполными данными, что '
    ...
    story.append(p(
        'VerdictEngine является ключевым компонентом Руководитель проекта, обеспечивающим взвешенную агрегацию '
    ...
    story.append(p(
        'В режиме поиска лотов Руководитель проекта координирует обработку каждого тендера с поддержкой параллельных '
        'веток и отката. После регистрации документов Делопроизводителем и предварительной оценки '
        'Закупщиком, Руководитель проекта направляет данные ПТО для извлечения ВОР. При наличии достаточных данных '
        ...
        'оценку. При обнаружении ошибок или критических расхождений Руководитель проекта инициирует откат через '
        'SK_ROLLBACK, восстанавливая состояние из revision_history.'
    ))
    story.append(p(
        'В режиме сопровождения строительства Руководитель проекта отслеживает этапы проекта, инициирует генерацию '
    ...
    story.append(p(
        'В версии v11.3.0 ПТО получил confidence_score для каждой позиции ВОР, что позволяет Руководитель проекта '
    ...
    story.append(p(
        ...
        'Юрист (асинхронно после Закупщика) - Руководитель проекта (вердикт через VerdictEngine). Каждый этап '
    ...
    story.append(p(
        ...
        'финансовую оценку. Результаты Юриста агрегируются в AgentState и используются Руководитель проекта на '
        'этапе формирования вердикта. Условное ветвление: если Закупщик определяет, что НМЦК ниже '
        '70% расчётной себестоимости, ПТО пропускается и Руководитель проекта формирует вердикт "не подавать" '
    ...
    construction_steps = [
        ...
        [Paragraph('Эскалация неоплаты', TABLE_CELL_STYLE),
         Paragraph('Руководитель проекта (3 напоминания без ответа)', TABLE_CELL_STYLE),
         Paragraph('Руководитель проекта, Юрист, Делопроизводитель', TABLE_CELL_STYLE),
    ...
    agent_state_fields = [
        ...
        [Paragraph('verdict_report', TABLE_CELL_STYLE),
         ...
         Paragraph('Да', TABLE_CELL_CENTER),
         Paragraph('Итоговый вердикт от Руководитель проекта', TABLE_CELL_STYLE)],
    ...
    story.append(p(
        ...
        'стоимость митигации риска в рублях, что позволяет Сметчику и Руководитель проекта количественно '
    ...
    story.append(p(
        ...
        'компонентов, а Руководитель проекта LLM-роутинг + VerdictEngine перенесены с P7 на P2, поскольку '
    ...
    roadmap = [
        ...
        [Paragraph('P2', TABLE_CELL_CENTER),
         Paragraph('Руководитель проекта LLM-роутинг + VerdictEngine (взвешенная модель, вето, эскалация). Бывший P7 - повышен!', TABLE_CELL_STYLE),
         Paragraph('Руководитель проекта', TABLE_CELL_STYLE),
        ...
        [Paragraph('P12', TABLE_CELL_CENTER),
         Paragraph('Human-in-the-loop интерфейс: веб-UI для Руководитель проекта Escalation, подтверждение/отклонение вердиктов, ручной review при confidence < 0.7', TABLE_CELL_STYLE),
         Paragraph('Руководитель проекта + UI', TABLE_CELL_STYLE),
         Paragraph('Планируется', TABLE_CELL_STYLE)],
