#!/usr/bin/env python3
"""
MAC ASD v11.2 — Навыки агентов и рабочие процессы (v4).
...
class CoverPage(Flowable):
    ...
    def draw(self):
        ...
        meta_values = ['v11.2.3', 'Апрель 2026', 'Mac Studio M4 Max 128GB / MLX', 'LangGraph + LLMEngine + pgvector']
...
def build_content():
    ...
    story.append(p(
        'Ключевым нововведением версии v11.2.3 является VerdictEngine - взвешенная модель принятия '
    ...
    story.append(Paragraph('Таблица 1. Обзор агентов MAC ASD v11.2.3', CAPTION_STYLE))
    ...
    story.append(p(
        'В версии v11.2.3 Hermes дополнен VerdictEngine - взвешенной моделью принятия решений, '
    ...
    story.append(p(
        'В версии v11.2.3 ПТО получил confidence_score для каждой позиции ВОР, что позволяет Hermes '
    ...
    story.append(p(
        'В версии v11.2.3 Сметчик получил поддержку версионирования ФЕР: система способна '
    ...
    story.append(p(
        'Агент Юрист является одним из наиболее развитых агентов в текущей реализации (v11.2.3). '
    ...
    story.append(p(
        'В версии v11.2.3 структура БЛС расширена новыми полями: precedent_id (ссылка на дело в '
    ...
    story.append(p(
        ...
        'и выбор поставщиков на основе данных от Логиста. В версии v11.2.3 Закупщик получил '
    ...
    story.append(p(
        ...
        'В версии v11.2.3 Логист получил навык автоматического обновления рейтингов поставщиков '
    ...
    story.append(p(
        'Архитектура v11.2.3 вводит параллельные ветки выполнения для оптимизации времени обработки '
    ...
    story.append(p(
        ...
        'В версии v11.2.3 добавлена эскалационная лестница для неоплаченных счетов, автоматизация '
    ...
    story.append(p(
        ...
        'могут отсутствовать при определённых условиях (например, при пропуске ПТО). Версия v11.2.3 '
    ...
    story.append(p(
        'БЛС (База Ловушек Подрядчика) в версии v11.2.3 содержит 58 ловушек, распределённых '
        'по 10 категориям. Это значительное расширение по сравнению с версией v11.2.2, которая '
    ...
    story.append(Paragraph('Таблица 13. Категории БЛС v11.2.3 (58 ловушек, 10 категорий)', CAPTION_STYLE))
    story.append(Spacer(1, 12))

    story.extend(add_h2('12.2. Новые поля'))
    story.append(p(
        'В версии v11.2.3 структура записей БЛС расширена тремя новыми полями, значительно '
    ...
    story.append(p(
        'Категория "Процедурные ловушки" является новой в v11.2.3 и охватывает риски, связанные '
    ...
    story.append(p(
        'В версии v11.2.3 добавлена стратегия MLX offloading: при нехватке памяти для '
    ...
    story.append(Paragraph('Таблица 14. Стек технологий MAC ASD v11.2.3', CAPTION_STYLE))
    story.append(Spacer(1, 12))
    ...
    story.append(p(
        ...
        'которые не могут быть обработаны через PyMuPDF на Stage 1. В версии v11.2.3 Stage 2 '
    ...
    story.append(p(
        ...
        'и приоритеты бизнес-логики. Версия v11.2.3 существенно пересматривает приоритеты: '
        ...
        'Юрист уже реализован (v11.2.2), но его расширение запланировано на P5.'
    ))
    ...
    story.append(Paragraph('Таблица 15. Дорожная карта разработки v11.2.3', CAPTION_STYLE))
